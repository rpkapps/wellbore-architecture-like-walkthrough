...
import { chip, h, slider } from '../ui/dom';
...
export class CurtainFeature implements FeatureModule {
  ...
  settings() {
    ...
    for (const c of CURVES) {
      ...
      sel.append(h('option', { value: c.key, disabled: !ok }, `${c.label}${ok ? '' : ' — not on this well'}`));
    }
    sel.value = this.curve;
    sel.onchange = () => {
      this.curve = sel.value;
      this.rebuild();
      this.app.featuresPanel.renderSettings('curtain');
    };
    const wd = slider({ label: 'Curtain height', min: 10, max: 200, step: 5, value: this.width, format: (v) => `${v} m`, onInput: (v) => ((this.width = v), this.rebuild()) });
    const def = CURVE_BY_KEY.get(this.curve)!;
    return h('div', {}, h('div', { class: 'row' }, h('label', {}, 'Log'), sel), wd.el, h('div', { class: 'feat-note', html: `${chip(def.prov)} ${def.label}, ${def.range}. Width of the fill = value; colour = value.` }));
  }
