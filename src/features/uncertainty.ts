...
export class UncertaintyFeature implements FeatureModule {
  ...
  settings() {
    ...
    for (const k of [1, 2, 3]) sel.append(h('option', { value: String(k) }, `${k}σ (${k === 1 ? '68' : k === 2 ? '95' : '99.7'}% for a 1-D error)`));
    sel.value = String(this.sigma);
    sel.onchange = () => {
      this.sigma = +sel.value;
      this.rebuild();
    };
    ...
    return h(
      'div',
      {},
      h('div', { class: 'row' }, h('label', {}, 'Ellipse size'), sel),
      h(
        'div',
        { class: 'feat-note' },
        `Simplified systematic MWD model: σinc ${m.sigInc}°, σazi ${m.sigAzi}°, depth ${m.sigDepth * 1000} ‰ of MD${w?.trajectory.status === 'reconstructed' || w?.trajectory.status === 'user' ? ', plus a reconstruction allowance growing away from the picks' : ''}. Not a full ISCWSA tool-code model.`,
        last ? h('div', {}, `At TD (${fmt.n(last.md, 0)} m): ${fmt.n(last.major * this.sigma, 1)} × ${fmt.n(last.minor * this.sigma, 1)} m across hole, ±${fmt.n(last.vertical * this.sigma, 1)} m vertical.`) : '',
      ),
    );
  }
