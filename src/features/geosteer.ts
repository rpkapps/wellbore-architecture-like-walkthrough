...
export class GeosteerFeature implements FeatureModule {
  ...
  private readout = h('div', { class: 'gs-readout' });
  private hud = h('div', { class: 'gs-hud hidden' });
  private lastMd = -1;
  ...
  constructor(private app: App) {
    ...
    this.targetSel = h('select', { class: 'select', title: 'Target formation' }) as HTMLSelectElement;
    for (const hz of app.field.horizons) {
      const f = FORMATION_BY_ID.get(hz.id);
      if (!f || !app.field.horizons[app.field.horizons.indexOf(hz) + 1]) continue;
      this.targetSel.append(h('option', { value: hz.id }, f.name));
    }
    ...
    const srcSel = h('select', { class: 'select', title: 'Surfaces' }) as HTMLSelectElement;
    srcSel.append(h('option', { value: 'tied' }, 'Tied to this well’s picks'), h('option', { value: 'model' }, 'Regional model only'));
    srcSel.onchange = () => {
      this.source = srcSel.value as SurfaceSource;
      this.rebuild();
    };
    const viewSel = h('select', { class: 'select', title: 'Strip extent' }) as HTMLSelectElement;
    viewSel.append(h('option', { value: 'window' }, 'Whole lateral'), h('option', { value: 'cursor' }, '±250 m of cursor'));
    viewSel.onchange = () => {
      this.view = viewSel.value as 'window' | 'cursor';
      this.draw();
    };
    this.panel = new FloatingPanel({
      ...
      height: 190,
      place: 'dock-bottom',
      headExtra: [this.targetSel, srcSel, viewSel],
      onClose: () => app.flags.set('geosteer', false),
    });
    this.panel.body.append(this.readout, this.canvas);
    this.panel.onResize = () => this.draw();
    this.canvas.addEventListener('click', (e) => {
      const md = this.mdAtX(e.offsetX);
      if (md !== null) app.travelTo(md);
    });
    app.hudSlot.append(this.hud);
  }

  enable() {
    this.app.engine.scene.add(this.group);
    this.panel.show();
    document.body.classList.add('gs-strip');
  ...
  disable() {
    ...
    this.panel.hide();
    document.body.classList.remove('gs-strip');
  ...
  settings() {
    return h(
      'div',
      { class: 'feat-note' },
      'Tied mode corrects the regional picks model so it passes through every formation boundary this well crossed — as a geosteerer would. Distances are vertical (TVD). Status: ',
      h('b', { style: `color:${STATUS_COLOR.in}` }, 'in zone'),
      ' · ',
      h('b', { style: `color:${STATUS_COLOR.above}` }, 'above top'),
      ' · ',
      h('b', { style: `color:${STATUS_COLOR.below}` }, 'below base'),
    );
  }
  ...
  private updateReadout() {
    ...
    const md = this.app.engine.rig.md;
    if (!p || !p.window) {
      this.readout.innerHTML = `<span class="faint">This well does not reach the ${FORMATION_BY_ID.get(this.target)?.name ?? this.target} in the model.</span>`;
      this.hud.innerHTML = '';
      return;
    }
    ...
    const where = s
      ? `<b style="color:${STATUS_COLOR[s.status]}">${status}</b> <span>${s.dTop >= 0 ? `${fmt.n(s.dTop, 1)} m below top` : `${fmt.n(-s.dTop, 1)} m above top`} · ${s.dBase >= 0 ? `${fmt.n(s.dBase, 1)} m above base` : `${fmt.n(-s.dBase, 1)} m below base`}${unc ? ` · ±${fmt.n(unc, 1)} m vertical (2σ)` : ''}</span>`
      : '';
    this.readout.innerHTML = `<div>${inWin ? where : `<span class="faint">Cursor above the landing section — scrub to MD ≥ ${fmt.n(p.window.from, 0)} m</span>`}</div><div class="gs-foot"><span style="color:${STATUS_COLOR.in}">in ${fmt.n(p.footage.in, 0)} m (${pc(p.footage.in)})</span><span style="color:${STATUS_COLOR.above}">above ${fmt.n(p.footage.above, 0)} m</span><span style="color:${STATUS_COLOR.below}">below ${fmt.n(p.footage.below, 0)} m</span></div>`;
    this.hud.innerHTML = inWin && s ? `<i style="background:${STATUS_COLOR[s.status]}"></i>${status} · top ${s.dTop >= 0 ? '+' : ''}${fmt.n(s.dTop, 1)} m · base ${s.dBase >= 0 ? '+' : ''}${fmt.n(s.dBase, 1)} m` : '';
  }
