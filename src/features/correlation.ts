...
import { chip, fmt, h, toggle } from '../ui/dom';
...
export class CorrelationFeature implements FeatureModule {
  ...
  constructor(private app: App) {
    const sel = (title: string, opts: [string, string][], value: string, on: (v: string) => void) => {
      const s = h('select', { class: 'select', title }) as HTMLSelectElement;
      for (const [v, l] of opts) s.append(h('option', { value: v }, l));
      s.value = value;
      s.onchange = () => on(s.value);
      return s;
    };
    const datumSel = sel(
      'Flatten on a formation top',
      [['', 'No flattening'], ...MODEL_HORIZONS.filter((id) => !['nordland'].includes(id)).map((id) => [id, `Flatten: ${FORMATION_BY_ID.get(id)?.name ?? id}`] as [string, string])],
      this.datumId,
      (v) => {
        this.datumId = v;
        this.win = null;
    ...
    const modeSel = sel('Vertical axis', [['tvdss', 'TVDSS'], ['md', 'MD']], this.mode, (v) => {
      this.mode = v as CorrDepthMode;
      this.win = null;
      this.rebuild();
    });
    const curveSel = sel('Filled curve', CURVES.map((c) => [c.key, c.label]), this.curve, (v) => {
      this.curve = v;
      this.rebuild();
    });
    const overSel = sel('Overlay curve', [['', 'No overlay'], ...CURVES.map((c) => [c.key, `+ ${c.label}`] as [string, string])], this.overlay, (v) => {
      this.overlay = v;
      this.rebuild();
    });
    this.panel = new FloatingPanel({
      ...
      height: 440,
      place: 'center',
      headExtra: [datumSel, modeSel, curveSel, overSel],
      onClose: () => app.flags.set('correlation', false),
    });
    this.panel.body.append(this.canvas, this.readout);
    ...
    this.canvas.addEventListener('pointerleave', () => this.hover(-1, -1));
    this.canvas.addEventListener('dblclick', () => ((this.win = null), this.draw()));
  ...
  settings() {
    const wrap = h('div', {});
    const ordSel = h('select', { class: 'select' }) as HTMLSelectElement;
    for (const [v, l] of [
      ['we', 'West → east'],
      ['sn', 'South → north'],
      ['list', 'Well list order'],
    ])
      ordSel.append(h('option', { value: v }, l));
    ordSel.value = this.order;
    ordSel.onchange = () => {
      this.order = ordSel.value as typeof this.order;
      this.rebuild();
    };
    wrap.append(h('div', { class: 'row' }, h('label', {}, 'Well order'), ordSel));
    for (const w of this.candidates())
      wrap.append(
        toggle(w.name, !this.hidden.has(w.id), (v) => {
    ...
    wrap.append(
      h(
        'div',
        { class: 'feat-note' },
        'TVDSS shows each well’s first downward pass, so laterals are compressed; switch to MD to see them in full. Wheel scrolls, Ctrl + wheel zooms, double-click resets. Click a track to travel there (other wells become active).',
      ),
    );
    return wrap;
  }
  ...
  private hover(x: number, y: number) {
    ...
    if (!p) {
      this.readout.textContent = this.cols.length ? 'Hover a track for values · click to travel' : '';
    ...
    this.readout.textContent = `${p.col.well.name} · MD ${fmt.n(p.md, 1)} m · ${this.mode === 'tvdss' ? 'TVDSS' : 'MD'} ${fmt.n(p.d, 1)} m${rel} · ${z?.name ?? ''}${val(this.curve)}${val(this.overlay)}`;
  ...
  draw() {
    ...
    this.cols.forEach((c, i) => {
      const x = left(i) + tw / 2;
      g.font = `${c.well === active ? 600 : 500} 11px Inter Variable, sans-serif`;
      g.fillStyle = c.well === active ? '#7fe3ff' : 'rgba(231,236,241,0.9)';
