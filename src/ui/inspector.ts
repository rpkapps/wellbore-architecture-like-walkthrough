...
type Row = [string, string, string?] | 'sep' | { h: string };

export class Inspector {
  readonly el: HTMLElement;

  constructor(private app: App) {
    this.el = h('div', { class: 'inspector glass hidden' });
  ...
  private render(title: string, sub: string, color: string, rows: Row[], desc?: string, actions: HTMLElement[] = [], badge?: string) {
    this.el.innerHTML = '';
    const close = h('button', { class: 'btn icon ghost', style: 'margin-left:auto;flex:none', html: I.close, onclick: () => this.hide() });
    this.el.append(
      h(
        'header',
        {},
        h('div', { class: 'sw', style: `background:${color}` }),
        h('div', { style: 'min-width:0' }, h('h4', {}, title), h('p', {}, sub), badge ? h('div', { style: 'margin-top:6px', html: badge }) : null),
        close,
      ),
    );
    const kv = h('div', { class: 'kv' });
    for (const r of rows) {
      if (r === 'sep') kv.append(h('div', { class: 'sep' }));
      else if ('h' in (r as object)) kv.append(h('div', { class: 'h micro' }, (r as { h: string }).h));
      else {
        const [k, v, prov] = r as [string, string, string?];
        kv.append(h('div', { class: 'k' }, k), h('div', { class: 'v' }, v), h('div', { class: 't', html: prov ? chip(prov, provShort(prov)) : '' }));
      }
    }
    this.el.append(kv);
    if (desc) this.el.append(h('div', { class: 'desc', html: desc }));
    if (actions.length) this.el.append(h('div', { class: 'actions' }, ...actions));
    ...
    switch (p.kind) {
      ...
      case 'detailWell': {
        ...
        return this.render(
          ...
          w.trajectory.note,
          [h('button', { class: 'btn primary', onclick: () => this.app.selectWell(w.id) }, 'Open this well')],
        );
      }
      case 'platform': {
  ...
  wellAt(md: number) {
    ...
    this.render(
      ...
      f?.color ?? '#7fe3ff',
      rows,
      f ? `<b>${f.name}</b> — ${f.description}` : undefined,
      [
        h('button', { class: 'btn primary', onclick: () => app.travelTo(md) }, 'Travel here'),
        h('button', { class: 'btn', onclick: () => app.logs.setCursor(md) }, 'Centre logs'),
      ],
    );
  }

  private casing(c: CasingString, cement = false) {
    ...
    this.render(
      ...
      cement
        ...
        : c.note,
      [],
      chip(cement ? 'schematic' : 'reconstructed'),
    );
  }

  formation(id: string, point?: { x: number; y: number; z: number }, top?: HorizonGrid, base?: HorizonGrid | null) {
    ...
    this.render(
      ...
      f.color,
      rows,
      `${f.description}<br><span class="faint">Surface interpolated from Equinor formation picks (plane trend + inverse-distance residuals); exact at the picks, interpretive between wells.</span>`,
      [
        h('button', { class: 'btn', onclick: () => { geo.isolate(geo.isolatedId === id ? null : id); this.app.left.sync(); } }, geo.isolatedId === id ? 'Clear isolation' : 'Isolate'),
        h('button', { class: 'btn', onclick: () => { geo.setLayer(id, { visible: false }); this.app.left.sync(); this.hide(); } }, 'Hide'),
      ],
    );
  }

  private fracture(fr: Fracture) {
    this.render(
      'Natural fracture',
      ...
      [],
      chip('schematic', 'Schematic — not measured'),
    );
  }
  ...
  zone(z: Zone) {
    ...
    this.render(f?.name ?? z.name, `${w.name} · formation top`, f?.color ?? '#888', rows, f?.description, [
      h('button', { class: 'btn primary', onclick: () => this.app.travelTo(z.topMD + 1) }, 'Travel to top'),
    ]);
  }

  private pay(iv: { top: number; base: number }) {
    const w = this.app.engine.activeWell;
    const p = w.petro!;
    ...
    this.render(
      ...
      [h('button', { class: 'btn primary', onclick: () => this.app.travelTo((iv.top + iv.base) / 2) }, 'Travel here')],
    );
  }
}

function provShort(p: string) {
  return { measured: 'M', calculated: 'C', interpreted: 'I', reconstructed: 'R', schematic: 'S', user: 'U', definitive: 'M' }[p] ?? p;
}
