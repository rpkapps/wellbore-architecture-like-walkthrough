...
**Workspace.** The 3D view fills the window and never changes size; the panels lie over it (`src/ui/workspace/`). Scene, Properties, Interpretation, Well logs and every tool window are panels in tabbed groups in three fixed regions: the left sidebar, the right sidebar and the bottom panel. Each sidebar has a top slot and a bottom slot with a draggable divider between them (Properties sits below Scene by default; an empty bottom slot hides), the bottom panel has one slot, and nothing creates further regions or splits. Dragging a tab only reorders it within its group (let go anywhere else and it snaps back); a panel moves between regions and slots from its tab's ⋯ menu or the rail's right-click menu (*Move to left sidebar / right sidebar / bottom panel*, *Move to top / bottom of … sidebar*). It floats only when undocked with the group's **↗** button (or *Undock* in those menus): a floating window moves by its header and resizes from its edges and corners, stays inside the stage, and is never docked by dragging; its **↙** button or a double click on its header docks it back into the slot and tab it came from. Each panel remembers its last floating size and position. Every move, undock, dock back, close and reset location shows a toast with **Undo**, and **Ctrl Z** takes back the last layout change. Drag region edges and a sidebar's divider to resize; fold a region to a strip of icons whose panels open as flyouts. The **panel rail** down the left edge lists every panel with a label: Scene, Properties, Interpretation and Well logs have their own entries, **Views** lists every analysis window by workflow phase (Explore, Interpret, Steer, Model, Monitor, Present): opening a view turns its feature on, and closing its window turns it off again so it stops computing, **Data** imports files and opens Production and the live sources, and Settings, Help and Hide panels sit at the bottom. A filled dot marks a panel that shows, a hollow one a panel that is open but hidden. Clicking an entry brings the panel up wherever it is (its tab to the front, its window raised, or reopened where it was; a feature's window turns its feature on); clicking a showing panel closes it, or folds its column when it is all the column holds; a panel in a folded column with others opens as a flyout (the folded left column has no strip of its own: the rail stands in for it). Right-click an entry to move it to another region or slot, undock or dock it back, reset its location or close it. Tabs always show their labels (the ones behind the active tab drop to icons only when the header runs out of room), each group's **+** opens a closed panel as a tab of that group, and a double click on a tab or **Ctrl Space** in a panel maximises its group over the stage (the rail and the timeline stay; Esc restores). The panel menu (⋯) moves, undocks, docks back, resets and maximises from the keyboard, in headed parts that say what each acts on: the panel showing (headed with its name), all the tabs of its group, and the sidebar or bottom panel (fold), and **Tab** (with nothing focused) hides every panel. A sidebar can be dragged wide enough to fill the window. The projection centre follows the area the panels leave free, so the subject stays centred without resizing the canvas. Only what belongs to the view floats over it: the **colour key** at the top right, which is also the colour-by control (its title is the *Resistivity ▾* menu; `V` still cycles), and the **viewport toolbar** at the bottom centre: *Guided | Explore*, the cameras for that mode, then the view tools (Measure, Saved views, Snapshot) and Field overview, folded into a ⋯ menu where the view is narrow. Where the camera is lives in the timeline (see Features). Docking, folding and hiding panels, and the colour key, details and chapter cards collapsing to one-line chips, animate with React's `<ViewTransition>` (the frosted glass of translucent panels is carried by the moving box, so it never drops out mid-animation). Dragging a window or a column edge moves the DOM directly and renders nothing until you let go.

**Workspaces** are tabs next to the well picker (Walkthrough, Petrophysics, Geosteering, then your own; **Ctrl PgUp / PgDn** steps through them). Each keeps its own live layout, Blender style: panel changes save into the active workspace as you make them, and switching away and back finds it as you left it (tool windows stay open while their feature is on and take each workspace's place for them). A workspace can also set up its task when you switch to it: Petrophysics colours the well by hydrocarbons, Geosteering follows the bit in guided chase view. Right-click a tab, or use the ⋯ beside the tabs, to *Reset* it (a built-in one to its preset, one of yours to the layout it was created with), *Duplicate*, *Rename* or *Delete* it (your own only); **+** starts a new workspace from the active one.

**Selection and Properties.** Clicking an object in the 3D view or a row of the Scene tree selects it (`app.selection`: a well, formation, pick, contact, overlay or interval; Escape clears it). The tree highlights the selected row and gains a *Wells* folder and an *Overlays* folder (log curtain, oil–water contact, uncertainty cones, anti-collision, geosteering band: the eye shows or hides each, the folder's eye all of them); the **Properties** panel (under the Scene group by default) shows the selected object's details with provenance, a filter for them, and its settings: a formation's colour, visibility, opacity and isolation, a scene layer's visibility, an overlay feature's own settings. Its pin keeps it on one object while you select others, to compare them. While Properties is closed or hidden, the floating details card over the 3D view shows the selection instead. Right-click a tree row or an object in the 3D view (or use a row's ⋮) for its actions (the browser's own right-click menu is off, except in text fields and over selected text): they come from the action registry, where each action names the kinds of object it applies to (`appliesTo`).

**Task bar, linked views and empty states.** With something selected, a small **task bar** floats just above it in the 3D view (docked above the viewport toolbar when the object has no place on screen) with its likely next steps, each opening a view already set up for it: for a well *Show logs · Correlate · Section along well · Geosteer* (opening that well first), for a formation *Isolate · Crossplot zone* (the crossplot limited to that formation) *· Flatten on top* (the correlation flattened on its top) *· Geosteer target*, for a formation top or an interval *Fly to · Show in logs*; ⋯ holds the rest of its actions and × (or Esc) clears the selection. A view whose feature is off is turned on. The steps are actions (`views.*` in `src/actions/appActions.tsx`), curated per kind in `src/ui/taskbar.ts`. The bar fades out while the camera moves or the walk plays and comes back beside the object; *Settings › Show the task bar for the selection* (or ⋯ › Hide) turns it off. Each analysis window's header has a **link chip** saying what it follows (*F-11 B · depth cursor*); pressing it opens switches for the links that view has, and the section and the crossplot can be **pinned** to their well, so they stay on it while another well is opened (two views can then compare wells). The crossplot's brushed samples are shared as a **marking** (`app.marking`) that the timeline draws as thin ticks in the marking colour above the strip; clicking one travels there. A view with nothing to show for the open well says why and offers one step instead of a blank canvas: a crossplot without the curves it needs names a well that has them (*Switch to 15/9-F-11 A*) or a plot this well can make, the correlation with fewer than two logged wells offers more wells, geosteering a well that misses the target offers the formation it ends in, the simulation without its files offers the import, and the logs of a well without logs offer an import.

**Controls and look.** Numbers are scrub fields (drag sideways, click to type, arrow keys); the net-pay cut-offs and GR limits draw the well's own distribution inside the field. Text uses a small set of roles (titles, section headers, labels, values, captions) at a 15 px root size (14, 15 or 17 px with the density setting: Compact, Default, Roomy); the text drawn on canvases and in SVG (log-track headers and depth labels, tool-window plots, live charts, the timeline's scale) and the 3D labels grow with it, and their margins and header rows grow with the text (`textPx` / `textLen` in `src/ui/tokens.ts`); the accent colour marks only what must stand out (cursor, playhead, selection). **Settings** (at the foot of the rail) sets the theme (dark, light or system), density, accent colour, panel opacity and background blur, whether overlays start collapsed, scene-label density, the **graphics quality** (Low, Medium, High, or Custom with a switch per effect) and reduced motion; settings are kept in the browser. The resistivity colour map is chosen from the colour key itself: each map is previewed over the active well's own deep-resistivity log, and hovering one previews it live. Folders in the Scene tree have an eye for everything in them, and grouped switch lists (Settings › Graphics › Custom, the Well logs track editor) say how many are on with a quiet *All on* / *All off*; truncated text shows in full on hover. The 3D view draws on demand (camera motion, input, state changes) rather than every frame, and the page loader animates in a worker (OffscreenCanvas) so it keeps moving while the dataset is parsed.

**Actions and the command palette.** Every operation is an action (`src/actions/`): an id that doubles as a tool name, a title, a description, a Zod input schema, and whether it needs approval. The command palette (**⌘K / Ctrl K**, or the search button) lists them, drills into their choices (every well, formation, feature, workspace) and asks for values (go to depth, set a parameter); keys such as Space, V, N / P and 1–3 run the same actions. It also answers "where is it?": typing matches names first, then keywords and places, then descriptions ("porosity" finds the crossplot), and each result shows where it lives in the interface ("Scene › Display"); an empty field starts with what you ran recently and often; `>` lists every panel with its place ("Left column · tab 2", "Floating", "Hidden") and Enter brings it into view (turning its feature on if needed); `?` lists these modes and the help. The registry validates input and returns errors as values, `app.actions.describe()` gives JSON Schema tool descriptions, and the **Assistant** calls the same actions as tools, so it does exactly what the palette does (read-only `app.state` tells it what is on screen and what is selected); `src/actions/tanstack.ts` also turns them into TanStack AI client tools. An action with `appliesTo` also appears in the right-click menus: `app.actions.actionsFor(selection)` lists the ones that apply to the selected object with its id filled in (by convention as `{ formation: id }`, `{ well: id }`, or mapped by the action's `onSelection`).

**Assistant.** The **Assistant** panel (the sparkle button in the top bar, **⌘I / Ctrl I**, the rail's *AI* entry, or *Ask the assistant* on any selected object's right-click menu) is a chat with a model of your choice that can see and operate the app. Ask what you are looking at, for numbers from the data ("net pay by zone in this well", "monthly oil for all wells"), or for a change ("colour the well by hydrocarbons", "take me to the top of the Hugin"). It reads the app through the same actions as the palette plus read-only data tools (logs, tops, zones, pay, production, trajectory, contacts, the simulation summary). It answers with charts, depth plots, tables and metrics drawn in the chat (generated interfaces in the [A2UI](https://a2ui.org) protocol), bound to the exact data the tools returned. It attaches the selected object as context, and can take a capture of the 3D view for models that see images. By default every change waits for your *Approve* (the composer switches between *Read only*, *Ask first* and *Auto*). There is no server: connect a provider under the model menu › *Manage providers* (OpenAI, Anthropic, Google Gemini, DeepSeek, OpenRouter, Groq, Mistral, xAI, Together, Fireworks, Cerebras, a local Ollama or LM Studio, or any OpenAI-compatible endpoint) with your own key. The key stays in this browser, is sent only to that provider, and is kept for the session unless you ask the settings to remember it. Providers that refuse browser requests (CORS) can go through a proxy prefix set on the connection. Conversations are kept in the browser (IndexedDB), can be renamed, deleted and exported as Markdown, and `/` in the composer lists commands. To try it without a key, `pnpm assistant:demo` starts a scripted stand-in model (add a *Custom* connection with base URL `http://localhost:8787/v1`; the script's header lists what it understands). The chat is a portable kit in `src/assistant/` (see its README) and loads only when the panel first opens; the BoreWalk-specific tools, context and instructions are in `src/assistant-host/`.
...
- **Guided walkthrough** with data-driven chapters (rig floor → seabed → Utsira aquifer → casing shoes → sidetrack → chalk → Draupne source rock → landing → best pay → reservoir exit and re-entry → TD). Three views: *Inside* (tunnel camera in the hole), *Chase* (cutaway section) and *Orbit*, switched from the viewport toolbar (or 1–3). The full-width timeline under the window draws the whole hole (formations, inclination, pay, casing shoes) with the chapters as numbered stops; drag it to scrub, or use the arrow keys. Beside play and speed it reads out where you are (MD, TVDSS, inclination, formation and, with geosteering on, IN ZONE); click the read-out for the details (compass, camera, attitude, feature read-outs). The chapter card shows above its marker while the tour or the walk plays, or when you pick a chapter (click a marker, N / P); close it and the numbered markers remain. Includes play, speed control and an auto tour. The colour key and chapter card each collapse to a one-line chip.
- **Free exploration**: fly with WASD/QE, drag to look, Shift to boost, wheel to set speed. In the orbit view the mouse works as in Onshape, Fusion or Google Earth: left drag turns the view around the point under the pointer (a dot marks it; over empty space, around the view's centre), right or middle drag (or Shift + left) drags the point you grabbed, and the wheel zooms toward the point under the pointer by a share of the distance, so it slows near a surface (keep going and it passes through at a steady pace; rock faded near the camera is looked through, not zoomed into); zooming out, or zooming over empty space, moves straight along the view. The view's centre stays within the model and the camera within a few model sizes of it, so the field cannot be lost off screen. A released rotation glides only briefly. Double-click focuses on a point. You can go above, below and sideways from the well. Rock between the camera and what you are looking at (the well in Guided, the point you zoomed toward or turned around in Explore) thins steadily the closer it is to the camera, down to a trace of its depth contours, so it never blocks the view and never switches off in one go; changing mode or view blends it over about half a second.
- **Geomodel**: 12 horizons interpolated from multi-well picks (planar trend + inverse-distance residuals, forced into stratigraphic order), built as capped solids. A BIM-style **section box** is edited on a plan view of the field (top-reservoir relief, every wellbore, the camera's place on the well): drag its edges, corners or middle, and drag the cut on the formation column beside it to strip overburden to a TVDSS. The 3D block follows a drag every frame (the slabs are rewritten in their existing GPU buffers) and is rebuilt at the box's full resolution when you let go. You can show, hide, fade and **isolate** any formation, and use presets such as *Reservoir focus* and *Isolate pay*. The procedural PBR lithologies (sandstone, claystone, chalk with stylolites and flint, marl couplets, organic shale, coal-streaked delta plain, red beds) follow conformable bedding.
...
- **Log tracks synced with 3D**: GR/caliper, resistivity with the matching colour strip, density–neutron with crossover shading, sonic, calculated Vsh/φ, and Sw with So fill plus the CPI Sw overlay. Hovering highlights the depth in 3D, clicking travels there, the wheel scrolls and Ctrl+wheel zooms. The **track editor** (sliders button in the Well logs header, or *Edit log tracks* in ⌘K; a popover over the logs, which redraw as you change them) shows, hides and reorders tracks (drag the grip, or use the keyboard), edits each curve's scale, log/linear, direction and colour, and **adds tracks for any curve the well has** — every LAS curve (PEF, DRHO, ROP, the other resistivity curves, or any mnemonic in your own file), the Equinor CPI (KLOGH, PHIF, VSH, BVW, flags) or the calculated curves. New tracks get the conventional scale for known mnemonics and a rounded P2–P98 range otherwise (logarithmic when the data span decades). The layout is saved in the browser and applies to every well; added tracks are skipped on wells that lack their curves.
...
There is no panel of feature switches: each feature is found and used where it belongs (*Where* below; `home` in `src/features/registry.ts`).

- **Views** (Section, Correlation, Crossplot, Map, Simulation, the anti-collision travelling cylinder, and the Geosteering strip) open from the rail's **Views** menu, a group's **+**, the command palette or `>`. Opening one turns its feature on; closing its window turns a view off again, so it stops computing. A view's own settings (well order and which wells a correlation shows, notes on reading it) are behind the ⚙ in its header.
- **Overlays** in 3D (log curtain, oil–water contact, uncertainty cones, the geosteering band) are rows of the Scene tree's **Overlays** folder: the eye shows or hides one (hidden, it computes nothing), and selecting it shows its settings (curve, σ, contact plane…) in **Properties**. Clicking one in the 3D view selects it too: Properties names it, says how to read it and what it shows where you clicked (the log value on the curtain, the distances to the target's top and base on the band, the ellipse size on a cone), with *Show only this* to hide the other overlays. A click through an uncertainty cone that also meets the well picks the well. Closing the Geosteering window leaves its band in 3D; its eye hides both. **Anti-collision** scans the active well every 10 m from the seabed against every other wellbore with a trajectory (the surveyed wells and the context wellbores) and draws a line, coloured by separation factor, from the well to the other well at each close approach, with the offset wells listed by lowest SF in Properties (click one to travel there); hole shared with a parent or sidetrack above the kick-off is not counted. The **travelling cylinder** (Views › Steer) shows the same thing around the bit: looking down the active well at the depth cursor, high side up, every wellbore within 100 m projected onto the plane across the hole and coloured by separation factor, with the ellipse size and caution threshold shared with the overlay (which need not be on).
- **ROP** is offered in the colour key's *Colour by* menu whenever the active well has an ROP log.
- **More Volve wells**: *Show more Volve wells* / *Show fewer wells* at the foot of the well picker.
- **Graphics** effects: **Settings › Graphics › Quality**: *Low* (all four off, as `?q=low` starts), *Medium* (photo textures and sea detail on; shadows and inside-the-hole effects, the costliest per frame, off), *High* (all on, the default), or *Custom* with a switch for each.
- **Tools** (Measure, Saved views, Snapshot) are always on the viewport toolbar; they cost nothing until used.

The flags remain the mechanism underneath and are remembered in your browser; `?features=all`, `?features=none` or a list such as `?features=geosteer,curtain` in the URL overrides them. The command palette can set any of them (*Show Log curtain*, *Open Crossplot*, *Graphics quality*), turn them all on or off, or *Reset optional features to defaults*. Features marked *GPU* start off in the low-quality mode (`?q=low`).

| Feature | Where | What it does | Provenance | Default |
| --- | --- | --- | --- | --- |
| **Geosteering view** | Scene › Overlays (*Geosteering band*) · Views › Steer | Where the well is relative to the top and base of a target formation (Hugin by default) at every depth. In 3D: a status band on the hole (in zone / above / below), drop-lines to both surfaces, and the surfaces as ribbons along the path. A distance-to-boundary strip docks above the timeline. The timeline's position read-out shows the status (IN ZONE), and its details the distances to top and base. *Tied* mode corrects the smooth regional model so it passes through every formation boundary the well actually crossed, as a geosteerer would. F-11 B: 592 m of the lateral in the Hugin (39 %), matching its picks. | Calculated | on |
| **Log curtain** | Scene › Overlays | A ribbon hanging off the trajectory with GR, resistivity, So, φ, RHOB, NPHI, DT or ROP drawn as a filled, colour-coded curve. It stands vertically above laterals and turns sideways where the hole is vertical. | Measured / calculated | on |
| **Cross-section along the well** | Views › Explore | A 2D section along the unrolled well path: formations from the picks model, trajectory, casing shoes, tops and a log. It overlays tied surfaces, the oil–water contact and uncertainty when those features are on. Vertical exaggeration is selectable; click to travel there. | Interpreted | off |
| **Well correlation** | Views › Interpret | Log tracks of every logged well side by side, ordered west→east (or south→north), **flattened on a formation top** (Hugin by default) or hung in TVDSS / MD. Each track is shaded by formation, and the same formation is filled and its top joined between neighbouring wells, so thickening, thinning and missing section stand out. Pick the filled curve and an overlay curve (GR + deep resistivity by default). In TVDSS the panel shows each well's first downward pass, so laterals are compressed; switch to MD to see them in full. Wheel scrolls, Ctrl + wheel zooms, clicking a track travels there (switching the active well if needed). Choose which wells to show under the ⚙ in its header. | Measured (logs) / interpreted (tops) | off |
| **Crossplots linked to 3D** | Views › Interpret | Density–neutron (with the clean-matrix line from ρma and ρfl), **Pickett** (log Rt vs log φ with the Sw = 1 water line and iso-Sw lines from a, m, n and Rw) and Buckles (φ vs Sw with bulk-volume-water hyperbolas and the pay cut-offs) for the active well, over the reservoir, one formation or the whole log. Points are coloured by formation, GR, Sw or depth. The lines move live as you change the Interpretation parameters, so you can calibrate Rw and m against the wet sands by eye. Hovering a point lights up its depth on the borehole, clicking travels there, and **dragging a box marks those samples along the well in 3D** and lists them as depth intervals. | Measured / calculated | off |
| **Oil–water contact** | Scene › Overlays | Deepest oil (Sw < 0.5) and shallowest water (Sw > 0.8) in clean Hugin–Skagerrak sands of every logged well, from the live Sw calculation. Each bracket is drawn as a disc at its well, with an optional field-wide plane at the median, which you can set yourself. The wells do **not** share one contact (F-12 ≈ 2913 m, F-11 A ≈ 3025 m, F-14 ≈ 3059 m, F-5 ≈ 3148 m TVDSS), which is consistent with fault compartments and logging at different stages of depletion. The feature says so rather than forcing one plane. | Calculated | on |
| **Drilling speed (ROP) mode** | Colour key › Colour by | A fourth colouring mode from the LAS ROP curve, with footage-weighted mean ROP and on-bottom drilling hours per formation. | Measured | on |
| **Survey uncertainty cones** | Scene › Overlays | 1σ/2σ/3σ position ellipses swept along the path from a simplified systematic MWD model (σinc 0.1°, σazi 0.5°, 1 ‰ depth), plus an allowance on trajectories reconstructed from picks. The vertical band also appears in the geosteering strip and cross-section. Not a full ISCWSA tool-code model. | Calculated | off |
| **Well anti-collision** | Scene › Overlays (*Anti-collision*) | The active well against every other wellbore (surveyed wells and context wellbores), every 10 m from the seabed: the closest point on each, and a separation factor SF = centre-to-centre distance ÷ (kσ + kσ), σ the major semi-axis of each well's uncertainty ellipse. Under 1.0 collision risk, under the caution threshold (1.5, or 1.25 / 2.0) caution. In 3D a coloured line joins the well to the other well at each close approach, with a label at the closest one and the risky stretches marked along the hole; Properties lists the offset wells by lowest SF, each with a button that travels there. A sidetrack's hole shared with its parent above the kick-off (from the paths and the listed kick-off) is reported as shared, not as a risk. F-11 B at 2σ: F-12 about 6 m away at 1,236 m MD (SF 1.03, caution), F-7 7.5 m at 1,066 m (SF 1.16), the reconstructed F-12 pilot hole SF 0.99 (its path from picks carries a wide allowance); F-11 A and F-11 T2 share its hole to about 2,600 m. A simplified, conservative sum-of-radii factor, not a full ISCWSA scan (no hole sizes, along-hole or surface-position error). | Calculated | off |
| **Anti-collision travelling cylinder** | Views › Steer | A polar plot looking down the active well at the depth cursor, high side up (north up where the hole is near vertical), angles clockwise from high side, rings at 5–100 m on a square-root scale so near wells get room. Every other wellbore whose closest point is within 100 m is drawn ±150 m along its own hole, projected onto the plane across the reference, coloured by separation factor (as the anti-collision overlay computes it), with a dot and a label (well · distance · SF) at its closest point; the well's own kσ ellipse is the shaded disc at the centre. Beside it the wells around the bit by SF; hole shared with a sidetrack's parent is named, not drawn. σ and the caution threshold are the overlay's, set from either. Click a well to travel to its closest approach. F-11 B at 1,240 m MD, 2σ: F-12 about 6 m away (SF ≈ 1.05, caution), the F-1 family about 79 m. | Calculated | off |
| **More Volve wells** | Well picker | Adds F-1 A, F-1 B, F-14, F-15 D, F-4 and F-5 with logs, CPI, definitive surveys and daily production. | Measured | off |
| **Reservoir simulation** | Views › Model | Equinor's Volve simulation model on the map. Every active cell is an instanced box coloured by **oil saturation or pressure on a date slider (2008–2016, with play)**, or by porosity, permeability, NTG, region or depth. Filters for layers and values, clipping to the section box, and a chart of simulated vs reported field oil rate. The results come from re-running the Equinor deck with the open-source OPM Flow simulator (the original Eclipse restart files are not public). Cumulative oil to Oct 2016 is 10.88 M Sm³ in OPM, 9.98 M in Equinor's Eclipse run and 10.06 M reported. Import your own `.bwsim` / `.bwsim.gz` in Data. | Calculated | off |
| **Map view** | Views › Explore | A plan view of the field: a colour-filled **structure map** of any horizon with depth contours, every well path (detailed and context wells) with the point where it meets that horizon, the platform, the section box, the camera position and heading, and the **oil–water contact line** where the calculated contact meets the horizon. **Production bubbles** (cumulative or monthly rate, amber oil and blue water, blue rings for water injectors) sit where each well meets the horizon, on a date slider with play (2008–2016). Wheel zooms, drag pans, double-click resets, and clicking a well travels along it. | Interpreted (surfaces) / measured (production) | off |
| **Realistic textures** | Settings › Graphics | CC0 photo-scanned PBR textures from [Poly Haven](https://polyhaven.com) (albedo, normal, AO, roughness, metalness) for every rock type, the seabed, the borehole wall, the casing steel and the cement. Switch it on or off live, in Settings › Graphics or *Scene › Display › Realistic textures*, to compare with the procedural look. Slabs and the borehole wall use scale-aware world-space triplanar mapping, so repetition never shows as a grid. Casing and cement use a cylindrical mapping. The photos are tinted to each formation's colour and keep the modelled bedding. Credits: [`public/textures/CREDITS.md`](public/textures/CREDITS.md). `scripts/fetch_textures.py` rebuilds them (ambientCG assets also supported). | — | on (GPU) |
| **Shadows & ambient occlusion** | Settings › Graphics | Sun shadow map, plus a screen-space AO pass written for three.js' logarithmic depth buffer (the stock SSAO/GTAO passes assume linear depth). | — | on (GPU) |
| **Inside-the-hole atmosphere** | Settings › Graphics | Radial lens depth-of-field and drifting drilling-fluid particles in the Inside view. | — | on (GPU) |
| **Sea surface & seabed detail** | Settings › Graphics | Multi-octave waves with Fresnel sky reflection and sun glint, and sand ripples with shell-hash patches on the seabed. | — | on (GPU) |
| **Measure tool** | Viewport toolbar | Press **M**, then click two points: 3D distance, horizontal and vertical offsets, azimuth, TVDSS of both ends, and ΔMD when both points are on the well. | — | on |
| **Saved views & presentation** | Viewport toolbar | Bookmark camera, view mode, property, layers and section box with a caption; drag to reorder. **Present** plays them full-screen with captions (→ / Space next, ← back, Esc exit). *Add starter tour* builds five views for the active well. | — | on |
| **High-resolution snapshot** | Viewport toolbar | 2× or 4K PNG of the current view, rendered off-screen, with the 3D labels, a title block, colour bar, north arrow and data credits drawn on top. | — | on |
...
The dialog previews a connection before it touches any well: the columns as read, and what it would add (survey, tops, production, logs by depth, readings by time), with suggested steps. Rows that name a well go to that well, and a well BoreWalk does not have is created. A well being drilled grows in 3D (with **Follow the bit** on, in the Live data panel's header, the view goes to the bit and stays with it, in Guided or Explore without switching between them; turning it off stops the view where it is, and scrolling the logs or travelling along the well turns it off), its logs fill in behind the sensors, and readings by time show as strip charts in **Live charts**. **Replay a well live** drills a Volve well again, up to 3600× faster, as a rig feed through the same pipeline: a demo, and the test bench.
...
src/ui/workspace/               the panel workspace: layout model, fixed regions, floating windows, menus
src/actions/                    the action registry, the app's actions and the TanStack AI tool adapter
src/assistant/                  the assistant kit (portable, no app imports): agent loop, providers, A2UI renderer, chat panel
src/assistant-host/             BoreWalk's side of the assistant: tools, data tools, context, instructions, lazy panel
